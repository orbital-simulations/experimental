...
use glam::UVec2;
use wgpu::{TextureFormat, Texture, Operations, Color, RenderPass, CommandEncoder, RenderPassColorAttachment, RenderPassDepthStencilAttachment, TextureView, TextureViewDescriptor};

use crate::context::Context;

pub struct RenderTargetDescription<'a> {
    pub name: String,
    pub multisampling: u32,
    pub depth_texture: Option<(Rc<Texture>, Operations<f32>)>,
    pub targets: &'a [(Rc<Texture>,Operations<Color>)],
}
...
impl RenderTarget {
    pub fn new(context: &Context, parameters: &RenderTargetDescription) -> Self {
        ...
        if parameters.multisampling > 1 {
            ...
            let multisampling_textures = parameters.targets.iter().map(|(texture, _)|{
